...
for cmd in curl awk sed find tar date; do command -v "$cmd" >/dev/null 2>&1 || { echo "ERROR: $cmd is required."; exit 1; }; done
...
if [[ "$OS_ID" != "ubuntu" && "$OS_ID" != "debian" ]]; then
  echo "WARNING: This tool is designed for Ubuntu/Debian. Detected: $OS_ID"
fi

load_all(){ mapfile -t ALL_LINES < <(cat "$IRAN_FILE" "$FOREIGN_FILE" 2>/dev/null | awk -F'|' 'NF>=4 && $1 !~ /^#/ {print}'); }
load_all
...
check_mirror(){
 ...
 elapsed=$(( (end-start)/1000000 )); ((elapsed<1)) && elapsed=1
...
test_selected(){
 ...
 local raw="$TMPDIR/smart-mirror-results.$$"
 : > "$raw"
 echo "Starting tests..."
